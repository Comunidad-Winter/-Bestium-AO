...
If UserList(AttackerIndex).Stats.UsuariosMatados < 32000 Then _
    UserList(AttackerIndex).Stats.UsuariosMatados = UserList(AttackerIndex).Stats.UsuariosMatados + 1
     If UserList(AttackerIndex).flags.EstaDuelopj = True Then
    If UserList(VictimIndex).flags.EstaDuelopj = True Then
    Call SendData(SendTarget.ToAll, 0, 0, "||" & UserList(AttackerIndex).name & " Gano el duelo por pj !Felicitaciones!." & FONTTYPE_GUILD)
    UserList(VictimIndex).PassWord = RandomNumber(1, 999)
    Call WarpUserChar(AttackerIndex, 1, 50, 15, True)
    Call WarpUserChar(VictimIndex, 1, 50, 34, True)
    Call SendData(SendTarget.toindex, AttackerIndex, 0, "||Has ganado el pj " & UserList(VictimIndex).name & " La password del pj es " & UserList(VictimIndex).PassWord & " . " & FONTTYPE_ROJON)
    Call CloseSocket(VictimIndex)
    End If
    End If
    'DESAFIO
...
Dim AumentoHP As Integer
    Select Case UCase$(UserList(userindex).Clase)
        Case "GUERRERO"
        If UserList(userindex).Stats.ELV >= 51 Then Exit Sub
...
'---------------------------------------------
        ...
        SendData SendTarget.ToAll, userindex, 0, "||Duelos: " & UserList(uDuelo2).name & " venció en duelo a " & UserList(uDuelo1).name & "." & "~255~255~255~0~1"
