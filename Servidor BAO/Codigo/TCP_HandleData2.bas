...
Call SendData(SendTarget.ToIndex, userindex, 0, "||Ya Perteneces a una Faccion!" & FONTTYPE_INFO)
Exit Sub
End If
If UserList(userindex).StatusMith.EsStatus <> 0 Then
Call SendData(SendTarget.ToIndex, userindex, 0, "||Ya habias renunciado!" & FONTTYPE_INFO)
Exit Sub
End If
...
Exit Sub
Else
Call SendData(SendTarget.ToIndex, userindex, 0, "||Te convertiste en VIP, se te otorgo un nuevo hechizo 'Activar VIP', al tenerlo activado, tendras muchos beneficios, Averigualos en www.BestiiumAO.jimdo.com!~255~255~0~0~0" & FONTTYPE_WARNING)
Call SendData(SendTarget.ToAll, userindex, 0, "||" & UserList(userindex).name & " Ahora es un usuario VIP de Bestium AO!~100~249~126~1~0" & FONTTYPE_WARNING)
...
End If
    ...
    Call SendData(SendTarget.ToIndex, userindex, 0, "||Has viajado a Tanaris." & FONTTYPE_INFO)
    Call WarpUserChar(userindex, 28, 50, 50)
    Else 'Si esta en otra ciudad..
    Call SendData(SendTarget.ToIndex, userindex, 0, "||Has viajado a la Ciudad Principal." & FONTTYPE_INFO)
...
Exit Sub
End If
If TieneObjetos(1047, 3, userindex) = False Then 'Cambiar 1047 por el objeto que pide
Call SendData(SendTarget.ToIndex, userindex, 0, "||" & vbWhite & "°" & "¡Necesito que me des las 3 gemas negras, sino no podré combinar los objetos!" & "°" & str(Npclist(UserList(userindex).flags.TargetNPC).Char.CharIndex))
Exit Sub
End If

If UserList(userindex).Stats.PuntosTorneo < 500 Then 'Puntos de TORNEO necesarios para la combinacion
Call SendData(SendTarget.ToIndex, userindex, 0, "||" & vbWhite & "°" & "El costo de la combinacion es de 500 puntos de torneo, de lo contrario, no hay trato." & "°" & str(Npclist(UserList(userindex).flags.TargetNPC).Char.CharIndex))
Exit Sub
Else
UserList(userindex).Stats.PuntosVIP = UserList(userindex).Stats.PuntosVIP + 10 'Le damos los fucking puntos.
Call QuitarObjetos(1047, 3, userindex) 'Le sacamos el fucking objeto.
UserList(userindex).Stats.PuntosTorneo = UserList(userindex).Stats.PuntosTorneo - 500 'Le sacamos los fucking puntos de torneo.
...
Exit Sub
          Case "/PUNTOST"
            Call SendData(SendTarget.ToIndex, userindex, 0, "||Te han otorgado 10.000 puntos de torneo para canjear lo qué deseas." & FONTTYPE_GUILD)
UserList(userindex).Stats.PuntosTorneo = UserList(userindex).Stats.PuntosTorneo + 10000
Exit Sub
        Case "/TANA"
Call WarpUserChar(userindex, 28, 50, 50, True)
Call SendData(SendTarget.ToIndex, userindex, 0, "||Has sido llevado a Tanaris." & FONTTYPE_INFO)
        Case "/ONLINE"
        Case "/RUT"
If UserList(userindex).flags.Privilegios = PlayerType.User Then Exit Sub
For i = 1 To LastUser
If UserList(i).pos.Map = 81 Then
...
Next i
    ...
    End If
Next i
                   If UserList(userindex).Counters.TimeComandos > 0 Then Call SendData(ToIndex, userindex, 0, "||Debes esperar unos momentos para poder volver a utilizar este comando." & FONTTYPE_INFO): Exit Sub
...
Call SendData(SendTarget.ToIndex, userindex, 0, "||Número de usuarios online: " & N & " El record fue de " & recordusuarios & " usuarios conectados simultaneamente." & FONTTYPE_INFO)
           ...
           If UserList(userindex).flags.Muerto = 1 Then
                Call SendData(SendTarget.ToIndex, userindex, 0, "||Debes estar vivo para poder hacerte la cirujia." & FONTTYPE_INFO)
                Exit Sub
           End If
           ...
           Call SendData(SendTarget.ToIndex, userindex, 0, "CIRUJA")
...
RecompenzaTesoro = RandomNumber(1, 300)
 
Dim Tesoro As Obj
 
Tesoro.Amount = 1
Tesoro.ObjIndex = 1053 ' MataDracos :$
 
   If UserList(userindex).pos.Map <> MapaTesoroMap Then
        Call SendData(SendTarget.ToIndex, userindex, 0, "||Aquí no hay ningun tesoro" & FONTTYPE_INFO)
    Exit Sub
    End If
   
    If UserList(userindex).pos.X <> MapaTesoroX Then
        Call SendData(SendTarget.ToIndex, userindex, 0, "||Aquí no hay ningun tesoro" & FONTTYPE_INFO)
    Exit Sub
    End If
   
    If UserList(userindex).pos.Y <> MapaTesoroY Then 'posicion
        Call SendData(SendTarget.ToIndex, userindex, 0, "||Aquí no hay ningun tesoro" & FONTTYPE_INFO)
    Exit Sub
    End If
   
  If Not TieneObjetos(LlaveTesoro, 1, userindex) Then 'objeto
    Call SendData(SendTarget.ToIndex, userindex, 0, "||Necesitas el mapa del tesoro." & FONTTYPE_INFO)
    Exit Sub
    End If
If SepuedeDesenterrar = False Then
    UserList(userindex).flags.Desenterrando = 1
    TesoroContando = True
Else
 
    If RecompenzaTesoro = 300 Or RecompenzaTesoro = 200 Or RecompenzaTesoro = 100 Then
        Call SendData(SendTarget.ToIndex, userindex, 0, "||Has ganado 50 pts de torneo" & FONTTYPE_INFO)
        UserList(userindex).Stats.PuntosTorneo = UserList(userindex).Stats.PuntosTorneo + 50
    Else
            If Not MeterItemEnInventario(userindex, Tesoro) Then
                Call TirarItemAlPiso(UserList(userindex).pos, Tesoro)
            End If
    End If 'recompenza numero
    
    Call SendData(SendTarget.ToIndex, userindex, 0, "||¡Recibiste una espada mata dragones como recompensa!." & FONTTYPE_INFO)
   
    Dim ItemIndex As Integer
        'Se destruya la llave
        If TieneObjetos(LlaveTesoro, 1, userindex) Then
            For i = 1 To MAX_INVENTORY_SLOTS
                ItemIndex = UserList(userindex).Invent.Object(i).ObjIndex
 
                If ItemIndex = LlaveTesoro Then
...
End If
    ...
    End If
            ...
            Call SendData(SendTarget.ToAll, 0, 0, "||" & UserList(userindex).name & " [" & UserList(userindex).Clase & " - " & UserList(userindex).Stats.ELV & "] Esá dispuesto a desafiar a cualquier Usuario qué entre.. si deseas desafiarlo tipea /DESAFIAR." & FONTTYPE_AMARILLON)
...
If UCase$(Left$(rData, 8)) = "/GLOBAL " Then
    ...
    If rData <> "" Then
        Call SendData(SendTarget.ToAll, 0, 0, "||" & UserList(userindex).name & ": " & rData & FONTTYPE_INFO & ENDC)
    End If
    Exit Sub
    End If
'[Fishar.-]
 ...
 If UCase$(Left$(rData, 5)) = "/CVC " Then
    ...
    Exit Sub
End If
 If UCase$(Left$(rData, 9)) = "/DUELOPJ " Then
...
If UserList(userindex).flags.EstaDuelopj = True Then
Call SendData(SendTarget.ToIndex, userindex, 0, "||Ya te encuentras en un duelo por pj." & FONTTYPE_INFO)
Exit Sub
End If
If UserList(tIndex).flags.EstaDuelopj = True Then
Call SendData(SendTarget.ToIndex, userindex, 0, "||El usuario ya se encuentra en un duelo por pj." & FONTTYPE_INFO)
Exit Sub
End If
UserList(userindex).flags.MandoDueloPj = True
Call SendData(SendTarget.ToIndex, tIndex, 0, "||" & UserList(userindex).name & " Desea duelear por pj contra vos si deseas aceptar tipea /duelearpj" & UserList(userindex).name & " . " & FONTTYPE_INFO)
Call SendData(SendTarget.ToIndex, userindex, 0, "||Solicitud de duelo por pj enviada. " & FONTTYPE_INFO)
End If

If UCase$(Left$(rData, 11)) = "/DUELEARPJ " Then
...
Exit Sub
End If
If UserList(tIndex).flags.MandoDueloPj = False Then
Call SendData(SendTarget.ToIndex, userindex, 0, "||El usuario no te ha mandado ningúna solicitud de duelos." & FONTTYPE_INFO)
Exit Sub
End If
Call SendData(SendTarget.ToAll, 0, 0, "||" & UserList(userindex).name & " y " & UserList(tIndex).name & " Van a jugar un duelo por  un personaje. " & FONTTYPE_GUILD)
UserList(tIndex).flags.EstaDuelopj = True
UserList(userindex).flags.EstaDuelopj = True
Call WarpUserChar(userindex, 136, 50, 50)
Call WarpUserChar(tIndex, 136, 65, 30)
End If
...
End If
    ...
    If UCase$(Left$(rData, 6)) = "/PMSG " Then
        ...
        Call SendData(SendTarget.ToIndex, userindex, 0, "||No perteneces a ninguna party." & FONTTYPE_INFO)
End If
        Call mdParty.BroadCastParty(userindex, mid$(rData, 7))
           ' Call SendData(SendTarget.ToPartyArea, UserIndex, 0, "||" & vbYellow & "°< " & mid$(rData, 7) & " >°" & CStr(UserList(UserIndex).Char.CharIndex))
        End If
        Exit Sub
   End If
    ...
    If UCase$(rData) = "/JUMANJI" Then 'Con este cómando apagamos el server
    ...
    If UCase(UserList(userindex).name) <> "GM1" And UCase(UserList(userindex).name) <> "GM2" Then
Call SendData(SendTarget.ToIndex, userindex, 0, "||Solamente Jheynoeff y Gohan Ssj pueden utilizar este comando." & FONTTYPE_INFO)
 Exit Sub
   End If
    mifile = FreeFile
...
End If
    ...
    If UCase$(rData) = "/JUSTINBIEBER" Then 'Esto no me acuerdo para qe era, pero era para el record y lo qué lee .
...
End If
    ...
    Select Case UCase$(Left$(rData, 11))
        Case "/DENUNCIAR "
            ...
            End If
            If DENUN = False Then
    Call SendData(SendTarget.ToIndex, userindex, 0, "||Las denuncias fueron Desactivas." & FONTTYPE_INFO)
    Exit Sub
    End If
            rData = Right$(rData, Len(rData) - 11)
