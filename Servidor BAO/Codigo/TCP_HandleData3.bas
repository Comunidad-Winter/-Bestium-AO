...
Exit Sub
End If
If UCase$(Left$(rData, 12)) = "/DENUNCIASON" Then
rData = Right$(rData, Len(rData) - 12)
Call SendData(SendTarget.ToAll, userindex, 0, "||DENUNCIAS ACTIVADAS POR EL ADMINISTRADOR " & UserList(userindex).name & " " & "~128~255~64~1~0")
DENUN = True
Exit Sub
End If
If UCase$(Left$(rData, 13)) = "/DENUNCIASOFF" Then
rData = Right$(rData, Len(rData) - 13)
Call SendData(SendTarget.ToAll, userindex, 0, "||DENUNCIAS DESACTIVADAS POR EL ADMINISTRADOR " & UserList(userindex).name & " " & "~255~64~64~1~0")
DENUN = False
Exit Sub
End If
