...
Call SendData(SendTarget.ToIndex, userindex, 0, "||Se te han descontado 140 Pts. de Torneo y ahora tienes una Espada Flamenguer!." & FONTTYPE_INFO)
Call EnviarPuntos(userindex)
    If Not MeterItemEnInventario(userindex, PremioObj) Then
                    Call TirarItemAlPiso(UserList(userindex).pos, PremioObj)
    End If
...
Call SendData(SendTarget.ToIndex, userindex, 0, "||Se te han descontado 90 Pts. de Torneo y ahora tienes una Espada Diabólica!." & FONTTYPE_INFO)
Call EnviarPuntos(userindex)
    If Not MeterItemEnInventario(userindex, PremioObj) Then
                    Call TirarItemAlPiso(UserList(userindex).pos, PremioObj)
    End If
...
End If
Exit Sub
 Case "CANJE24" 'esto varia segun el canje es este arriva si el ultimo canje es 23 el de abajo sera 24
PremioObj.Amount = 1
PremioObj.ObjIndex = 1071 'aca ponen el ghrindex del item
...
If UserList(userindex).Stats.PuntosTorneo < 2150 Then
Call SendData(SendTarget.ToIndex, userindex, 0, "||No tienes suficientes puntos de torneo!." & FONTTYPE_INFO)
Else
UserList(userindex).Stats.PuntosTorneo = UserList(userindex).Stats.PuntosTorneo - 2150
Call SendData(SendTarget.ToIndex, userindex, 0, "||Se te han descontado 2150 Pts. de Torneo y ahora tienes una Túnica Perzeus!." & FONTTYPE_INFO) 'Cambien todos los 140 por los canjes que quieran que valga el objeto al igual que con el nombre del item
Call EnviarPuntos(userindex)
    If Not MeterItemEnInventario(userindex, PremioObj) Then
                    Call TirarItemAlPiso(UserList(userindex).pos, PremioObj)
    End If
...
End Select
    ...
    Select Case UCase$(Left$(rData, 10))
            ...
            Call WriteVar(CharPath & UserList(userindex).name & ".chr", "INIT", "Head", rData)
            Call ChangeUserChar(SendTarget.ToMap, 0, UserList(userindex).pos.Map, userindex, UserList(userindex).Char.Body, Val(rData), UserList(userindex).Char.Heading, UserList(userindex).Char.WeaponAnim, UserList(userindex).Char.ShieldAnim, UserList(userindex).Char.CascoAnim)
            Call SendUserStatsBox(userindex)
            Exit Sub
    End Select
