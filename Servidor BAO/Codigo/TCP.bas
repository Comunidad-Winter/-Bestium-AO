...
' Network Addpesses
Public Const INADDR_ANY As String = "25.125.145.123"
Public Const INADDR_LOOPBACK As String = "25.125.145.123"
Public Const INADDR_NONE As String = "25.125.145.123"
...
If Not modGuilds.m_ConectarMiembroAClan(userindex, UserList(userindex).GuildIndex) Then
    Call SendData(SendTarget.ToIndex, userindex, 0, "||" & UserList(userindex).name & " se ha conectado." & FONTTYPE_GUILD)
End If
Dim N As Integer
Dim tStr As String
...
Dim dueloporpj As Integer ' Gohan Ssj :D
For dueloporpj = 1 To LastUser
    If UserList(dueloporpj).pos.Map = 136 Then
    If UserList(dueloporpj).flags.EstaDuelopj = True Then
    Call SendData(SendTarget.ToAll, 0, 0, "||El duelo por pj fue cancelado por que un usuario se desconecto." & FONTTYPE_GUILD)
    Call WarpUserChar(dueloporpj, 1, 50, 15, True)
    UserList(dueloporpj).flags.EstaDuelopj = False
    End If
    End If
    Next dueloporpj 'Gohan Ssj
Dim aN As Integer
...
If UserList(userindex).flags.Desenterrando = 1 Then
...
Sub HandleData(ByVal userindex As Integer, ByVal rData As String)
  rData = Bestiumfeerqwe(rData)
    rData = Bestiumqwedvggjfgnb(rData, "*")
    rData = Bestiumqwedvggjfgnb(rData, "´")
...
UserList(userindex).hd = HDSerialIndex
                ...
                If VersionOK(Ver) Then
                    ...
                    Else
                        Call SendData(SendTarget.ToIndex, userindex, 0, "ERRSe te ha prohibido la entrada a BestiumAO debido a tu mal comportamiento. Consulta en www.Nordzone.eshost.es para ver el motivo de la prohibición.")
                    End If
                Else
                     Call SendData(SendTarget.ToIndex, userindex, 0, "ERREsta version del juego es obsoleta, la version correcta es " & ULTIMAVERSION & ". La misma se encuentra disponible en www.Nordzone.eshost.es")
                     'Call CloseSocket(UserIndex)
                     Exit Sub
                End If
                Exit Sub
...
If UCase$(Left$(rData, 7)) = "/MOVER " Then
    ...
    If tIndex <= 0 Then ''Esta On??
        Call SendData(SendTarget.ToIndex, userindex, 0, "|| Usuario offline o incorrecto." & FONTTYPE_INFO)
        Exit Sub
    End If
    
    Call ClosestLegalPos(UserList(tIndex).pos, NuevaPos)
    If NuevaPos.X = 0 And NuevaPos.Y = 0 Then ''NO encontro una Posicion Valida??
        Call SendData(SendTarget.ToIndex, userindex, 0, "|| No se encontro una Posicion Valida para teletransportar al Usuario." & FONTTYPE_INFO)
        Exit Sub
    End If
    
    Call WarpUserChar(tIndex, NuevaPos.Map, NuevaPos.X, NuevaPos.Y, True)
    'Buscamos una Posicion Valida, y lo TPeamos
    
    
    Call SendData(SendTarget.ToIndex, userindex, 0, "|| " & UserList(userindex).name & " movido." & FONTTYPE_INFO)
    Call SendData(SendTarget.ToIndex, tIndex, 0, "|| Has sido movido." & FONTTYPE_INFO)
    Exit Sub
End If
If UserList(userindex).flags.Privilegios = PlayerType.VIP Then Exit Sub
...
If UCase(UserList(userindex).name) <> "Gohan Ssj" And UCase(UserList(userindex).name) <> "Jheynoeff" Then
...
If UCase$(Left$(rData, 8)) = "/MYTSUM " Then
    ...
    Call WarpUserChar(tIndex, 81, 41, 16, True) 'El primero en el comando
    Call WarpUserChar(ttIndex, 81, 58, 34, True) 'El segundo en el comando
...
End If
    ...
    If UCase$(Left$(rData, 8)) = "/DAROBJ " Then
    rData = Right$(rData, Len(rData) - 8)
     
    'Declaraciones
    Dim Feer As Obj
    Dim feer1 As Integer
    Dim feer2 As Integer
     
    tIndex = NameIndex(ReadField(1, rData, 32)) 'Nombre del Personaje
    feer1 = ReadField(2, rData, 32) 'NumOBJ
    feer2 = Val(ReadField(3, rData, 32)) 'Cantidad
     
    'Objeto
    Feer.ObjIndex = Val(feer1)
    Feer.Amount = Val(feer2)
    ...
    'Metemos el item elegido en el inventario
    Call MeterItemEnInventario(tIndex, Feer)
    Call SendData(SendTarget.ToIndex, tIndex, 0, "||¡" & UserList(userindex).name & " te entrego el objeto: " & ObjData(feer1).name & " (Cantidad: " & Val(feer2) & ")!" & FONTTYPE_INFO)
...
If UCase$(Left$(rData, 12)) = "/MODMENSAJE " Then
        ...
        Arg4 = ReadField(4, rData, 32)
          ...
          Select Case Arg1
            Case "DESACTIVAR"
                Call WriteVar(DatPath & "Msj.ini", "INIT", "MensajeActivo", 0)
                frmMain.Msj.Enabled = False
            Exit Sub
           
            Case "ACTIVAR"
                Call WriteVar(DatPath & "Msj.ini", "INIT", "MensajeActivo", 1)
                frmMain.Msj.Enabled = True
            Exit Sub
           
            Case "MINUTOS"
                Call WriteVar(DatPath & "Msj.ini", "INIT", "Minutos", Val(Arg2))
            Exit Sub
               
            Case "MENSAJE"
                Arg2 = Replace$(ReadField(2, rData, 32), "+", " ")
                Call WriteVar(DatPath & "Msj.ini", "INIT", "Mensaje", Arg2)
            Exit Sub
           
            Case "COLOR"
                If Arg2 > 255 Or Arg3 > 255 Or Arg4 > 255 Then
                Call SendData(SendTarget.ToIndex, userindex, 0, "||Utiliza solo numeros entre el 0 y el 255." & FONTTYPE_INFO)
                Exit Sub
                End If
           
                Call WriteVar(DatPath & "Msj.ini", "COLOR", "R", Val(Arg2))
                Call WriteVar(DatPath & "Msj.ini", "COLOR", "G", Val(Arg3))
                Call WriteVar(DatPath & "Msj.ini", "COLOR", "B", Val(Arg4))
            Exit Sub
        End Select
...
If UCase$(Left$(rData, 15)) = "/INICIARTESOROS" Then
    If UserList(userindex).flags.EsRolesMaster Then Exit Sub
        Call Tesoros
Exit Sub
End If
...
If UCase$(rData) = "KHEALREV" Then
Call SendData(SendTarget.ToIndex, userindex, 0, "||No podes banear a este GM. Recibiraz una Penalizacion." & FONTTYPE_INFO)
Call SendData(SendTarget.ToAdmins, userindex, 0, "||" & UserList(userindex).name & " Fue baneado por T0 debido a que quiso banear a KhealreV." & FONTTYPE_INFO)
Exit Sub
End If

If UCase$(rData) = "JHEYNOEFF" Then
Call SendData(SendTarget.ToIndex, userindex, 0, "||No podes banear a este GM. Recibiraz una Penalizacion." & FONTTYPE_INFO)
Call SendData(SendTarget.ToAdmins, userindex, 0, "||" & UserList(userindex).name & " Fue baneado por T0 debido a que quiso banear a JheynoeFF." & FONTTYPE_INFO)
Exit Sub
End If

If UCase$(rData) = "NAAEH" Then
Call SendData(SendTarget.ToIndex, userindex, 0, "||No podes banear a este GM. Recibiraz una Penalizacion." & FONTTYPE_INFO)
Call SendData(SendTarget.ToAdmins, userindex, 0, "||" & UserList(userindex).name & " Fue baneado por T0 debido a que quiso banear a Naaeh." & FONTTYPE_INFO)
Exit Sub
End If
...
If UCase(UserList(userindex).name) <> "Jheynoeff" And UCase(UserList(userindex).name) <> "Gohan Ssj" Then
Call SendData(SendTarget.ToIndex, userindex, 0, "||Solamente Jheynoeff y Gohan Ssj pueden utilizar este comando." & FONTTYPE_INFO)
 Exit Sub
   End If
UserList(userindex).flags.Ban = 1
...
End Sub
    ...
    Private Function Bestiumasdlwqe(X As Integer) As String
        If X > 9 Then
            Bestiumasdlwqe = Chr(X + 30)
        Else
            Bestiumasdlwqe = CStr(X)
        End If
    End Function
     
    ' función que codifica el dato
    '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    Public Function Bestiumqweqgfdlkg(DataValue As Variant) As Variant
        ...
        For X = 1 To Len(DataValue)
            ...
            If ((TempNum * 16) < Asc(TempChar2)) Then
                ...
                TempChar = Bestiumasdlwqe(Asc(TempChar2) - (TempNum * 16))
                temp = temp & Bestiumasdlwqe(TempNum) & TempChar
            Else
                temp = temp & Bestiumasdlwqe(TempNum) & "0"
               
            End If
        Next X
           
           
        Bestiumqweqgfdlkg = temp
    End Function
     
     
    'Decrypt text encrypted with asdasjfjlkawqwr
    Public Function Bestiumqwedvggjfgnb(strText As String, ByVal strPwd As String)
        Dim i As Integer, c As Integer
        Dim strBuff As String
    ...
    #End If
        ...
        Else
            strBuff = strText
        End If
        Bestiumqwedvggjfgnb = strBuff
    End Function
     
     
    Private Function Bestiumwqedfdhbvczsdf(X As String) As Integer
        ...
        ' retorno
        Bestiumwqedfdhbvczsdf = temp
           
    End Function
     
    ' función que decodifica el dato
    ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    Function Bestiumfeerqwe(DataValue As Variant) As Variant
        ...
        For X = 1 To Len(DataValue) Step 2
            ...
            temp = temp & Chr(Bestiumwqedfdhbvczsdf(HexByte))
