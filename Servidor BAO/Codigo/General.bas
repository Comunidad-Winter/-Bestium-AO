      ...
      End If
        ...
        If UserList(i).Counters.Saliendo Then
            ...
            If UserList(i).Counters.Salir <= 0 Then
                ...
                Call SendData(SendTarget.toindex, i, 0, "||Gracias por jugar Bestium Online" & FONTTYPE_INFO)
...
SendData SendTarget.ToAll, 0, 0, "||Contando..." & CuentaRegresiva - 1 & "~230~180~10~1~0"
Else
SendData SendTarget.ToAll, 0, 0, "||!YA!" & "~220~83~14~1~0"
End If
Dim ToL As Byte
ToL = CuentaRegresiva - 1
CuentaRegresiva = ToL
SendData SendTarget.ToAll, 0, 0, "CU" & ToL
End If
    ...
    If TesoroContando = True Then
        ...
        Dim IndexTes As Integer
        For IndexTes = 1 To LastUser
            If UserList(IndexTes).flags.Desenterrando = 1 Then
                Call SendData(SendTarget.toindex, IndexTes, 0, "||Estás excavando, en " & TiempoTesoro & " segundos podras desenterrar el Tesoro." & FONTTYPE_INFO)
        ...
        Next IndexTes
            ...
            If TiempoTesoro = 0 Then
                ...
                If UserList(IndexTes).flags.Desenterrando = 1 Then
                    Call SendData(SendTarget.toindex, IndexTes, 0, "||Ya puedes desenterrar." & FONTTYPE_INFO)
                End If
...
Public Sub ReyFeer()
    ...
    Feer.Map = 128 ' Cambiar por el numero del mapa
    ...
    If ReyON = 0 Then
    Call SendData(ToAll, 0, 0, "||El Espiritú del Antiguo Rey Ancalagon ha reaparecido en la Sala del Rey (Mapa 128).." & FONTTYPE_INFO)
...
End Sub
Public Sub Msjw()
   If 0 = Val(GetVar(DatPath & "Msj.ini", "INIT", "MensajeActivo")) Then
Exit Sub
Else
  Call SendData(ToAll, 0, 0, "||Servidor> " & GetVar(DatPath & "Msj.ini", "INIT", "Mensaje") & "~" & GetVar(DatPath & "Msj.ini", "COLOR", "R") & "~" & GetVar(DatPath & "Msj.ini", "COLOR", "G") & "~" & GetVar(DatPath & "Msj.ini", "COLOR", "B") & "~" & GetVar(DatPath & "Msj.ini", "TAMAÑO", "T") & "")
 End If
    End Sub
