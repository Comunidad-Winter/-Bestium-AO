    ...
    If FoundChar = 1 Then '  ¿Encontro un Usuario?
       ...
       If UserList(TempCharIndex).flags.AdminInvisible = 0 Or UserList(userindex).flags.Privilegios = PlayerType.Dios Then
                ...
                If EsNewbie(TempCharIndex) Then
                    Stat = Stat & " <NEWBIE>"
                End If
                If UserList(TempCharIndex).Stats.PuntosTorneo >= 0 Then
                        Stat = " PuntosT: " & UserList(TempCharIndex).Stats.PuntosTorneo & " "
                             End If
                ...
                'By AxelitÓh PrrúM.-
                If Stat = Stat & " (" & UserList(TempCharIndex).Stats.MinHP & "/" & UserList(TempCharIndex).Stats.MaxHP & ")" Then
                End If
'By AxelitÓh PrrúM.-
...
If UserList(TempCharIndex).flags.Privilegios > 3 Then
                        Stat = Stat & " [Administrador] <Sección Administración> ~255~255~255~1~0"
                ElseIf UserList(TempCharIndex).flags.Privilegios > 2 Then
                        Stat = Stat & " <Game Master> <Staff Bestium AO> ~0~128~128~1~0"
                ElseIf UserList(TempCharIndex).flags.Privilegios > 1 Then
                        Stat = Stat & " <Event Master> <Sección Torneo> ~191~128~0~1~0"
                ElseIf Criminal(TempCharIndex) Then
                        Stat = Stat & " <Ciudadano de Kahlimdor> ~255~0~0~1~0"
                ElseIf Ciudadano(TempCharIndex) Then
                        Stat = Stat & " <Ciudadano de Anvilmar> ~64~191~255~1~0"
